...
function ativarPedido(id, acao, telefone, total, pagamento, hora, entrega) {
    ...
    if (acao == 'Iniciar' && pedido_whatsapp == 'Sim') {  // Envio de mensagem quando o pedido é iniciado
        let a = document.createElement('a');
        a.target = '_blank';
        a.href = 'http://api.whatsapp.com/send?1=pt_BR&phone=' + telefone + '&text= *Novo Pedido* %0A Hora: *' + hora + '* %0A Total: R$ *' + total + '* %0A Entrega: *' + entrega + '* %0A Pagamento: *' + pagamento + '*';
        a.click();
    }
    ...
    $.ajax({
        ...
        success: function (mensagem) {
            ...
            if (split[0] == "Alterado com Sucesso") {
                listar();
                if (acao.trim() === 'Entrega' && pedido_whatsapp == 'Sim') {
                    let a = document.createElement('a');
                    a.target = '_blank';
                    a.href = 'http://api.whatsapp.com/send?1=pt_BR&phone=' + telefone + '&text= *Atenção*  %0A ' + texto + ' %0A *Total* R$ ' + total + ' %0A *Tipo Pagamento* ' + pagamento + ' %0A';
                    a.click();
